...
Phalcon\Mvc\Dispatcher 组件负责在MVC应用程序中负责实例化控制器以及执行所需操作的。

分发器
很多重要的过程发生在MVC工作流本身，尤其是在控制器部分。
这些工作发生在控制调度度期间，控制器文件的读取，加载，初始化，以及操作的执行。
如果一个action中的流程跳转到另一个控制器的controller/action上，控制调度器再次启动，为了更好的说明这一点，下面的例子将展示 Phalcon\Mvc\Dispatcher 内部执行的过程：
//Dispatch loop
while (!$finished) {
    $finished = true;
